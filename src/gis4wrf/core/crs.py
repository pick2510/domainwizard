...
class CRS(object):
    ...
    def lonlat_srs(self) -> osr.SpatialReference:
        ...
        # Note: EPSGTreatsAsLatLong() is not a reliable check here - it queries the
        # EPSG authority table for the CRS's defined axis order, which is meaningless
        # for a CRS built via SetGeogCS() with no EPSG code attached (it consistently
        # returns True on newer GDAL regardless of the actual axis order in use).
        # The line above (fix_axis_order -> SetAxisMappingStrategy) is what actually
        # controls the coordinate order used by Transform() calls, and is sufficient.
        return srs_out
