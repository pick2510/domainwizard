...
# Postpones evaluation of type annotations so that 'nc.Dataset' below can be used
# as a type hint without importing netCDF4 at module load time (see comment further
# down on why netCDF4 is imported lazily instead).
from __future__ import annotations

from typing import TYPE_CHECKING, Mapping, Tuple, List, Optional, Dict, Callable, Any
...
def convert_wrf_nc_var_to_gdal_dataset(
    ...
    # Imported lazily: importing netCDF4 loads its bundled native libraries
    # (curl/ssl/hdf5), which can conflict with QGIS's own copies if pulled in
    # eagerly at plugin-load time rather than only when actually needed.
    import netCDF4 as nc
    ds = nc.Dataset(path)
    try:
